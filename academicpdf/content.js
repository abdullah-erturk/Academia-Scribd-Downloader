  ...
  getCleanFilename: () => {
    try {
      ...
      try {
        ...
        console.warn('[SPD] URL Slug parsing failed for string, falling back to JSON-LD');
      }
...
const Interface = {
  ...
  draw: async (docId, isEmbed) => {
    ...
    if (warningPdfBtn) {
      warningPdfBtn.onclick = () => {
        AppState.activeBtnId = 'sdl-warning-pdf-btn';
        if (isAcademia) {
          executeAcademiaDownload();
        } else {
          executeScribdDownload();
    ...
    if (header) {
      header.onclick = (e) => {
        ...
        if (card) {
          // --- ANCHOR CONVERSION FIX ---
          // Convert bottom/right CSS lock to explicit top/left before any transition.
          // This prevents the 'bottom' anchor from clashing with the 'top' animation during maximize.
          ...
          if (!isExpanding) {
            // "KÜÇÜLMEDEN" HEMEN ÖNCE: Mevcut büyük hali hafızaya (memory) al
            overlay.dataset.memTop = overlay.style.top || overlay.getBoundingClientRect().top + 'px';
            overlay.dataset.memLeft = overlay.style.left || overlay.getBoundingClientRect().left + 'px';
            overlay.dataset.memHeight = card.offsetHeight + 'px';
            overlay.dataset.memWidth = card.offsetWidth + 'px';
          }
          ...
          // Buffer time for DOM updates
          setTimeout(() => {
            const viewportH = window.innerHeight;
            const viewportW = window.innerWidth;
            
            card.classList.remove('sdl-no-transition');

            if (isExpanding) {
              // BÜYÜTÜLÜRKEN: Hafızadaki (memory) pozisyona ve boyuta birebir geri dön
              const memTop = overlay.dataset.memTop;
              const memLeft = overlay.dataset.memLeft;
              const memHeight = overlay.dataset.memHeight || (Math.min(Math.max(card.scrollHeight || 0, 620), viewportH - 40) + 'px');
              const memWidth = overlay.dataset.memWidth || '340px';

              // Ekrana sığıp sığmadığını güvenlik amacıyla son kez test et
              const tTop = parseInt(memTop) || overlay.getBoundingClientRect().top;
              const tHeight = parseInt(memHeight) || 0;
              let safeTop = tTop;

              if (tTop + tHeight > viewportH - 10) {
                 safeTop = Math.max(10, viewportH - tHeight - 40);
              }

              overlay.style.top = safeTop + 'px';
              overlay.style.left = memLeft || overlay.style.left;
              card.style.height = memHeight;
              card.style.width = memWidth;
            } else {
              // KÜÇÜLTÜLÜRKEN: CSS .sdl-minimized class'ı yüksekliği zaten auto ve 240px yapıyor.
              // Biz sadece yeni oluşan tam net boyutu okuyoruz (padding, margin dahil)
              const targetHeight = card.offsetHeight;
              const targetWidth = 240;
              
              // --- MINIMIZE SNAPPING (Köşelere Yapışma) ---
              const rect = overlay.getBoundingClientRect();
              const centerX = rect.left + rect.width / 2;
              const newTop = viewportH - targetHeight - 20;
              let newLeft = 20; // Default: Bottom Left
              
              if (centerX > viewportW / 2) {
                // If it is mostly on the right side, snap to Bottom Right
