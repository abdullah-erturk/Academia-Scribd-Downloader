...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "fetch_image") {
        ...
        fetch(request.url)
            ...
            .catch(e => {
                console.error('[SPD-DEBUG] Fetch failed:', request.url, e.message);
    ...
    if (request.action === "fetch_text") {
        ...
        fetch(request.url)
            ...
            .catch(e => {
                console.error('[SPD-DEBUG] Text fetch failed:', request.url, e.message);
                sendResponse({ success: false, error: e.message });
            });
        return true;
    }
    ...
    if (request.action === "force_download") {
        const rawName = request.docName || 'Document';
