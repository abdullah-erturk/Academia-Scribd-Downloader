...
chrome.downloads.onDeterminingFilename.addListener((item, suggest) => {
    if (item.byExtensionId === chrome.runtime.id && expectedFilenames.has(item.url)) {
        suggest({ filename: expectedFilenames.get(item.url), conflictAction: 'uniquify' });
        expectedFilenames.delete(item.url);
        return true;
    }
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "capture_tab") {
        // Instant Active Check: Ensure the sender tab is still the active tab to prevent capturing other tabs
        chrome.tabs.query({ active: true, lastFocusedWindow: true }, (tabs) => {
            const activeTab = tabs[0];
            if (!activeTab || activeTab.id !== sender.tab.id) {
                console.warn('[SPD] Capture aborted: Sender tab is no longer active.');
                return sendResponse({ success: false, error: 'Tab is no longer active' });
            }
        ...
        fetch(request.url, { method: 'HEAD' })
            .then(response => {
                ...
                if (isValid) {
                    ...
                    // Force filename override to bypass Content-Disposition restrictions (Chrome specific issue)
                    expectedFilenames.set(request.url, finalName);
                    
                    chrome.downloads.download({ url: request.url, filename: finalName, saveAs: false });
    ...
    if (request.action === "force_download" || request.action === "download_pdf") {
        ...
        // Force filename override to bypass Content-Disposition restrictions (Chrome specific issue)
        expectedFilenames.set(urlToDownload, finalName);
